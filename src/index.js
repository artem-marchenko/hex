...
const getData = async () => {
  let res = await fetch('http://localhost:3000/styles');

  let content = await res.json();

  let list = document.querySelector('.comments__container');
    ...
    list.innerHTML += `
        <div class="comment">
            <div class="comment__image">
                <img src=${content[key].src} alt="">
            </div>
    
            <div class="comment__group">
                <div class='comment__body'>
                <span class="comment__name">${content[key].name}</span>
                <span class="comment__profession">${content[key].profession}</span>
                <p class="comment__text">${content[key].comment}</p>
                </div>
...
const forms = () => {
  ...
  const postData = async (url, data) => {
    document.querySelector('.status').textStatus = message.loading;
